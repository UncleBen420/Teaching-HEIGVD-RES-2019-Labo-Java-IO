...
public class Application implements IApplication {
	...
	public void fetchAndStoreQuotes(int numberOfQuotes) throws IOException {
		...
		for (int i = 0; i < numberOfQuotes; i++) {
			 ...
			 * web service client to fetch quotes. We have removed a single line from this
			 * method. It is a call to one method provided by this class, which is
			 * responsible for storing the content of the quote in a text file (and for
			 * generating the directories based on the tags).
			 */
	...
	void storeQuote(Quote quote, String filename) throws IOException {
		...
		// On cree le fichier et on ecrit dedant
		...
		Writer writer = Files.newBufferedWriter(Paths.get(directory + "/" + filename));
	...
	void printFileNames(final Writer writer) {
		...
		explorer.explore(new File(WORKSPACE_DIRECTORY), new IFileVisitor() {
			...
			public void visit(File file) {
				...
				// Modifie par Rémy Vuagniaux pour le Labo 2 RES
				// On dit au write d'ecrire le nom et le chemin du dossier
